(ns com.ben-allred.app-simulator.ui.simulators.shared.modals-test
  (:require
...
(deftest ^:unit request-modal-test
  (testing "(request-modal)"
    (with-redefs [dates/format (spies/constantly ::formatted)]
      (let [qp {:a 1 :b 2}
            rp {:id 123}
        ...
        (testing "when the path is '/'"
          ...
          (testing "formats the dt moment"
            (is (spies/called-with? dates/format ::moment))))
...
(deftest ^:unit socket-modal-test
  (testing "(socket-modal)"
    (with-redefs [dates/format (spies/constantly ::dt-string)]
...
(deftest ^:unit message-editor-test
  (testing "(message-editor)"
    (with-redefs [shared.views/with-attrs (spies/create identity)]
      (let [root (modals/message-editor ::form ::model->view ::view->model)
        ...
        (testing "renders a message field"
          (is (spies/called-with? shared.views/with-attrs (spies/matcher map?) ::form [:message] ::model->view ::view->model))
