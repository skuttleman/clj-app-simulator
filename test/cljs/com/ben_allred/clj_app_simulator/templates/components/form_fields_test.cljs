(ns com.ben-allred.clj-app-simulator.templates.components.form-fields-test
        ...
        (testing "when rendering a select component"
          (let [options [[{:a 1} "A"]
                ...
                attrs {:to-view    to-view
                       ...
                       :label      ::label
                       :errors     {:has :errors}}
                root (render ff/select attrs options)
            ...
            (testing "and when rendering without a label"
              (let [root (render ff/select (dissoc attrs :label) options)
                    form-field (test.dom/query-one root :.form-field)]
            ...
            (testing "and when :to-model is nil"
              (let [root (render ff/select (dissoc attrs :to-model) options)
                    form-field (test.dom/query-one root :.form-field)
                    select (test.dom/query-one form-field :select)]
      ...
      (with-redefs [dom/target-value target-value]
        (testing "when rendering a textarea component"
          (let [attrs {:to-view    to-view
            ...
            (testing "and when rendering without a label"
              (let [root (render ff/textarea (dissoc attrs :label))
                    form-field (test.dom/query-one root :.form-field)]
                (testing "does not render a :label element"
                  (is (not (test.dom/query-one form-field :label))))))

            (testing "and when rendering without errors"
              (let [root (render ff/textarea (dissoc attrs :errors))
                    form-field (test.dom/query-one root :.form-field)]
                (testing "does not have an :errors class"
                  (is (not (:class-name (test.dom/attrs form-field)))))))

            (testing "and when :to-view is nil"
              (let [root (render ff/textarea (dissoc attrs :to-view))
                    form-field (test.dom/query-one root :.form-field)
            ...
            (testing "and when :to-model is nil"
              (let [root (render ff/textarea (dissoc attrs :to-model))
                    form-field (test.dom/query-one root :.form-field)
                    textarea (test.dom/query-one form-field :textarea)]
                (testing "passes the target value to :on-change"
                  (spies/reset! on-change)
      ...
      (with-redefs [dom/target-value target-value]
        (testing "when rendering a input component"
          (let [attrs {:to-view    to-view
            ...
            (testing "and when rendering without a label"
              (let [root (render ff/input (dissoc attrs :label))
                    form-field (test.dom/query-one root :.form-field)]
                (testing "does not render a :label element"
                  (is (not (test.dom/query-one form-field :label))))))

            (testing "and when rendering without errors"
              (let [root (render ff/input (dissoc attrs :errors))
                    form-field (test.dom/query-one root :.form-field)]
                (testing "does not have an :errors class"
                  (is (not (:class-name (test.dom/attrs form-field)))))))

            (testing "and when :type is nil"
              (let [root (render ff/input (dissoc attrs :type))
                    form-field (test.dom/query-one root :.form-field)
            ...
            (testing "and when :to-view is nil"
              (let [root (render ff/input (dissoc attrs :to-view))
                    form-field (test.dom/query-one root :.form-field)
                    input (test.dom/query-one form-field :input)]
                (testing "has the unchanged :value"
            ...
            (testing "and when :to-model is nil"
              (let [root (render ff/input (dissoc attrs :to-model))
                    form-field (test.dom/query-one root :.form-field)
                    input (test.dom/query-one form-field :input)]
                (testing "passes the target value to :on-change"
                  (spies/reset! on-change)
      ...
      (with-redefs [dom/target-value target-value]
        (testing "when rendering a header component"
          (let [attrs {:to-view   to-view
            ...
            (testing "and when rendering without a label"
              (let [root (render ff/header (dissoc attrs :label))
                    form-field (test.dom/query-one root :.form-field)]
                (testing "does not render a :label element"
                  (is (not (test.dom/query-one form-field :label))))))

            (testing "and when rendering without errors"
              (let [root (render ff/header (dissoc attrs :errors))
                    form-field (test.dom/query-one root :.form-field)]
                (testing "does not have an :errors class"
                  (is (not (:class-name (test.dom/attrs form-field)))))))

            (testing "and when :to-view is nil"
              (let [root (render ff/header (dissoc attrs :to-view))
                    form-field (test.dom/query-one root :.form-field)
            ...
            (testing "and when :to-model is nil"
              (let [root (render ff/header (dissoc attrs :to-model))
                    form-field (test.dom/query-one root :.form-field)
                    header (test.dom/query-one form-field :.header-key)
                    value (test.dom/query-one form-field :.header-value)]
  ...
  (testing "(multi)"
    (let [key-spy (spies/create identity)
          ...
          attrs {:label ::label
                 ...
                 :class-name ::class-name
                 :value values}
          root (render ff/multi attrs :component)
          form-field (test.dom/query-one root :.form-field)]
              ...
              (testing "and when rending a remove-button"
                (let [button (test.dom/query-one item :.remove-item)]
                  (testing "handles :on-click"
                    ...
                    (let [[f] (first (spies/calls change-spy))
                          result (f values)]
                      (is (vector? result))
                      (is (->> values
                               (map-indexed vector)
