(ns com.ben-allred.clj-app-simulator.templates.components.form-fields
  (:require [com.ben-allred.clj-app-simulator.utils.dom :as dom]
...
(defn select [{:keys [on-change value class-name to-model to-view] :as attrs} options]
  (let [to-model (or to-model identity)
        to-view (or to-view identity)]
    [form-field
     attrs
     [:select
      {:on-change  #?(:clj identity :cljs (comp on-change to-model dom/target-value))
       :value      (to-view value)
...
(defn textarea [{:keys [on-change value class-name to-model to-view] :as attrs}]
  (let [to-model (or to-model identity)
        to-view (or to-view identity)]
    [form-field
     attrs
...
(defn input [{:keys [on-change value class-name type to-model to-view] :as attrs}]
  (let [to-model (or to-model identity)
        to-view (or to-view identity)]
    [form-field
     attrs
...
(defn header [{:keys [value on-change to-model to-view] :as attrs}]
  (let [to-model (or to-model identity)
        to-view (or to-view identity)
        [k v] (to-view value)]
    [form-field
     ...
     [:div.header-field
      [:input.header-key
       {:value     k
        :on-change #?(:clj identity :cljs #(on-change (to-model [(dom/target-value %) v])))}]
      [:input.header-value
       {:value     v
        :on-change #?(:clj identity :cljs #(on-change (to-model [k (dom/target-value %)])))}]]]))
