(ns com.ben-allred.clj-app-simulator.templates.views.resources
  (:require #?@(:cljs [[com.ben-allred.clj-app-simulator.ui.services.store.actions :as actions]
...
(defn resource [{:keys [id filename timestamp]}]
  [:li.resource
   ...
   [:div.resource-details
    ...
    [:div
     "Uploaded "
     [:span.resource-timestamp (dt/format timestamp)]]]
   [:div.button-row
    [components/upload
     {:on-change  #?(:clj identity :cljs (comp store/dispatch (partial actions/upload-replace id)))
      :class-name "button button-warning pure-button"
      :multiple   false}
     "Replace"]
    [:div
     [:button.button.button-error.pure-button.delete-button
      {:on-click (show-delete-modal "Delete Resource" "this resource" #?(:clj nil :cljs (actions/delete-upload id)))}
      "Delete"]]]])
...
(defn root [upload-welcome? uploads]
  [:div
   [:div.button-row
    [components/upload
     {:on-change  #?(:clj identity :cljs (comp store/dispatch actions/upload))
      :class-name "button button-success pure-button"}
     "Upload"]
    [:div
     [:button.button.button-error.pure-button.delete-button
      {:disabled (empty? uploads)
       :on-click (show-delete-modal "Delete All Resources" "all resources" #?(:clj nil :cljs actions/delete-uploads))}
      "Delete All"]]]
   (cond
     upload-welcome?
